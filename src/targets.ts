...
/** One saved remote development target. */
export interface SshTarget {
  ...
  passwordEnv: string;
  remoteDir: string;
  /** Local directory used as the session cwd when this target is opened. */
  localDir: string;
  createdAt: string;
  updatedAt: string;
}

export interface TargetStoreFile {
  version: 1;
  defaultId: string;
  targets: SshTarget[];
...
export function loadStore(): TargetStoreFile {
  const file = storePath();
  if (!existsSync(file)) return { ...EMPTY, targets: [] };
  try {
...
function isTargetish(value: unknown): value is SshTarget {
  return typeof value === 'object' && value !== null && typeof (value as SshTarget).host === 'string';
}

function normalizeStored(value: SshTarget): SshTarget {
  return {
...
export function saveStore(store: TargetStoreFile): void {
  const file = storePath();
...
/** Fields the Remote UI or the agent may submit for one target. */
export interface TargetInput {
  /** Existing target id or display name this request addresses. */
  target?: string;
  id?: string;
  name?: string;
  host?: string;
  ...
  remoteDir?: string;
  localDir?: string;
  /** Ask the host to make this target the default after saving. */
  setDefault?: boolean;
  /** Credential reference to use when resolving a one-off password. */
  path?: string;
}

/** Create or update one target, returning the stored record. */
export function upsertTarget(store: TargetStoreFile, input: TargetInput): SshTarget {
  const host = (input.host ?? '').trim();
  if (!host) throw new Error('host 不能为空');
  const port = Number.isFinite(input.port) ? Number(input.port) : 22;
  if (!(port >= 1 && port <= 65535)) throw new Error(`端口无效：${String(input.port)}`);
...
export function toSshConfig(target: SshTarget): SshConfig {
  return {
    host: target.host,
    port: target.port,
...
export function describeTarget(target: SshTarget) {
  return {
    id: target.id,
    name: target.name,
    host: target.host,
