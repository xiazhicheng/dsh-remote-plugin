...
function sshExec(client, command, timeoutMs) {
...
function sshRead(client, remotePath) {
  return new Promise((resolve, reject) => {
...
function sshWrite(client, remotePath, content) {
  return new Promise((resolve, reject) => {
    client.sftp((err, sftp) => {
      if (err) return reject(err);
...
export function apply(ctx, config = {}) {
  ...
  function registerSshTools(agent) {
    const disposers = [];
    try {
      ...
      ctx.logger.warn('retry-llm-plugin: failed to register SSH tools for agent: %s', e.message);
  ...
  const disposeAgentListener = ctx.on('agent/created', ({ agent }) => {
    registerSshTools(agent);
  });

  const disposeAgentDisposeListener = ctx.on('agent/disposed', ({ agent }) => {
