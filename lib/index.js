  ...
  async function registerSshTools(agent) {
    ...
    try {
      disposers.push(agent.ctx.tools.register(defineTool({
        ...
        execute: (args) =>
          getSshClient().then((c) => sshExec(c, args.command, args.timeoutMs ?? 30000)),
      })));
      ...
      disposers.push(agent.ctx.tools.register(defineTool({
        ...
        execute: (args) =>
          getSshClient().then((c) => sshRead(c, args.path)),
      })));
      ...
      disposers.push(agent.ctx.tools.register(defineTool({
        ...
        execute: (args) =>
          getSshClient().then((c) => sshWrite(c, args.path, args.content)),
      })));
    } catch (e) {
